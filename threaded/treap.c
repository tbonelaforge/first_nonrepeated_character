...
/*
The characters should be sorted first by priority ( number of occurrences )
and if there is a tie there, should then further be sorted by the index of
...
int is_dominated( struct threaded_binary_node * child, struct threaded_binary_node * parent ) {
    ...
    if ( child_data->priority > parent_data->priority ) {
        return 1;
    } else if ( child_data->priority == parent_data->priority ) {
        if ( child_data->first_index > parent_data->first_index ) {
            return 1;
        }
        else {
            return 0;
...
struct threaded_binary_node * get_highest_priority_child( struct threaded_binary_node * parent ) {
    ...
    if ( ( child = get_threaded_right_child( parent ) ) ) {
        if ( !highest_priority_child ) {
            ...
            if ( is_dominated( highest_priority_child, child ) ) {
                highest_priority_child = child;
            }
        }
    }
...
struct threaded_binary_node * insert( struct threaded_binary_node * root, char c, int first_index ) {
    struct treap_data * target_data = new_treap_data();
    target_data->value = c;
    target_data->priority = 1; // Assume c will not be found.
    target_data->first_index = first_index;
    int comparison = -1;
